...
function routePath(req, res){
    console.log("url:"+req.url);//获取完整url，例如/a?username=a&passwd=b

    //解析request的url，封装到pathObj中，提取出其中的pathname和query对象
    var pathObj = url.parse(req.url, true);
    console.log(pathObj.pathname);//例如/a
    console.log(pathObj.query);//例如 [Object: null prototype] { username: 'a', passwd: 'b' } 或空对象

    //根据pathname来匹配routes数组中的函数
    var handleFn = routes[pathObj.pathname];
    if(handleFn){  //如果route数组中有相应的匹配，则执行之
        //想要执行函数，则必须定义函数中的req.query，req.body.username和req.body.password参数
        //获取req.query：将query绑定到req.query上
        ...
        //声明一个body
        var body = '';
        //监听request的data事件，每次有data就添加到body中，
        req.on('data', function(chunk){
            body += chunk
        }).on('end', function(){//监听到end后，获得body，将body通过parseBody()解析为数组，并传递给req.body
            console.log("body:"+body)//例如 username=aaa&password=bbb
            req.body = parseBody(body);//解析body成数组
            console.log("reqbody:"+ req.body);//例如 { username: 'aaa', password: 'bbb' }
            handleFn(req, res);
        })

    }else {   //如果route数组中没有对应的值，则执行staticRoot函数
        staticRoot(path.join(__dirname, 'step2_static'), req, res)
    }
}
